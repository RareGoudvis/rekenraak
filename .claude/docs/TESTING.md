...
| `catalogue.test.ts` | `oefeningen.html` (generated by `scripts/catalogue.mjs`) carries exactly the `APP_STRUCTURE` leaves as `data-leaf` cards and every leaf has its `public/oefeningen/<leafId>.png` — a new or renamed leaf without `npm run catalogue` fails the gate; the Stop hook nudges earlier. |
...
`public/oefeningen/<leafId>.png`, then splices the result into the indexable
`oefeningen.html` (repo root, a Vite-built static page — see `vite.config.ts`
`build.rollupOptions.input` and `src/site.ts`) — the catalogue section, its sidebar
domain/subdomain/leaf nav, and the ItemList JSON-LD, each between their own
`<!-- catalogue…:start/end -->` marker comments; the page shell (chrome, intro copy)
sits outside those markers and is hand-edited. Every generated `<article class="ex-card">`
carries `data-leaf="<leafId>"` + `data-type="<typeId>"`, and every nav row carries
`data-leaf` too — a vitest guard diffs these against `APP_STRUCTURE` so a shipped exercise
type can't leave the catalogue stale. Before the screenshot, each leaf is deselected
(`setActiveSelection(null)`) so the png shows the plain white sheet, not the app's blue
selection highlight. Like the width matrix and font baseline it drives a real browser and
needs the DEV-only `window.__rekenraak` hook, so it cannot run against a production build.
